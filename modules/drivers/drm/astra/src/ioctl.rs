...
struct FramebufferRef {
    handle: u32,
    object: SharedObject,
    /// Stable pin information. ASTRA has no BO migration yet, so this is the
    /// same address/placement snapshot Linux keeps in prepared plane state.
    bo_gpu_address: u64,
    bo_size: usize,
    bo_place: Place,
    width: u32,
...
/// BO references owned by one scheduler job. Pointer identity is the GEM
/// object identity, so the collection maintains Linux's unique reservation
/// list invariant while its RAII ownership keeps every object alive to fence.
struct SubmissionObjects {
    entries: Vec<SharedObject>,
}
...
impl SubmissionObjects {
    ...
    fn contains(&self, object: &SharedObject) -> bool {
        self.entries
            .iter()
            .any(|current| Arc::ptr_eq(current, object))
    }

    fn insert(&mut self, object: SharedObject) -> Result<()> {
    ...
    fn iter(&self) -> core::slice::Iter<'_, SharedObject> {
        self.entries.iter()
    }
}
...
impl CursorBuffer {
    ...
    pub(crate) fn is_same_surface(&self, other: &Self) -> bool {
        Arc::ptr_eq(&self._object, &other._object)
            && self.gpu_address == other.gpu_address
            && self.pitch_pixels == other.pitch_pixels
    }
}
...
impl Adapter {
    fn fence_completed(&self, fence: crate::ip::CompletionFence) -> Result<bool> {
        Ok(self
            .wb
            .as_ref()
    ...
    fn wait_fence(&self, fence: crate::ip::CompletionFence, timeout: u64) -> Result<bool> {
        loop {
            if self.fence_completed(fence)? {
                return Ok(true);
            }
...
impl FileState {
    fn reap_submissions(&mut self, adapter: &mut Adapter) -> Result<()> {
        let mut retired_bo = false;
        let mut index = 0;
        while index < self.submissions.len() {
            if adapter.fence_completed(self.submissions[index].fence)? {
                retired_bo |= self.submissions[index]
                    .objects
                    .iter()
                    .any(|object| Arc::strong_count(object) == 1);
                // Sequence lookup is by value, not vector order. Avoid the
                // O(n) shift for every completed job on Mesa's CS hot path.
                self.submissions.swap_remove(index);
            } else {
                index += 1;
            }
        }
    ...
    fn wait_all_submissions(&mut self, adapter: &mut Adapter) -> Result<()> {
        for submission in &self.submissions {
            let _ = adapter.wait_fence(submission.fence, u64::MAX)?;
        }
        self.reap_submissions(adapter)
    }
}
...
impl FileState {
    pub fn new(gpu: &mut Gpu, file: FileId, vmid: u32) -> Result<Self> {
        Ok(Self {
            file,
            vm: UserVm::new(gpu, vmid)?,
            next_handle: 1,
    ...
    /// Consumes a DRM file's complete ownership tree.  BO lists, handles,
    /// contexts, submissions and VM page tables then retire through Drop;
    /// only the hardware VM unmap needs an explicit operation beforehand.
    pub fn close(mut self, gpu: &mut Gpu) -> Result<()> {
        self.wait_all_submissions(gpu)?;
        let mut first_error = None;
        while let Some(mapping) = self.mappings.pop() {
            let result = self.vm.clear_range(gpu, mapping.address, mapping.size);
...
impl UserVm {
    fn pte_flags(vm_flags: u32, bo_flags: u64, place: Place) -> u64 {
    ...
    fn alloc_table(gpu: &mut Gpu, clear_flags: u64) -> Result<Bo> {
        let adapter: &mut Adapter = gpu;
        let table =
            adapter
                .mem
                .alloc_vram_aligned(&mut adapter.regs, GPU_PAGE_SIZE, GPU_PAGE_SIZE)?;
        let dst = Self::table_gpu_addr(gpu, &table)?;
        ...
        gpu.update_vm_table(dst, 0, 512, 0, clear_flags)?;
        Ok(table)
    }
    ...
    fn new(gpu: &mut Gpu, vmid: u32) -> Result<Self> {
        ...
        // entries.
        let root = Self::alloc_table(gpu, PDE_PTE)?;
        let pdb1 = Self::alloc_table(gpu, PDE_PTE)?;
        let value = Self::table_pa(gpu, &pdb1)? | VRAM_TABLE_FLAGS;
        Self::write_entry(gpu, &root, 0, value)?;
        Ok(Self {
            vmid,
            root,
            pdb1,
    ...
    fn write_entry(gpu: &mut Gpu, table: &Bo, index: usize, value: u64) -> Result<()> {
        ...
        let dst = Self::table_gpu_addr(gpu, table)?
            ...
            .ok_or(Error::Range)?;
        gpu.update_vm_table(dst, value, 1, 0, 0)
    }

    fn ensure_ptb(&mut self, gpu: &mut Gpu, pdb1_index: u16, pdb0_index: u16) -> Result<usize> {
    ...
    fn clear_range(&mut self, gpu: &mut Gpu, va: u64, size: u64) -> Result<()> {
        let mut done = 0u64;
        while done < size {
            let current_va = va.checked_add(done).ok_or(Error::Range)?;
            let (pdb1_index, pdb0_index, pte_index) = Self::indices(current_va);
            ...
            {
                let dst = Self::table_gpu_addr(gpu, &ptb.page)?
                    .checked_add((pte_index * 8) as u64)
                    .ok_or(Error::Range)?;
                gpu.update_vm_table(dst, 0, count, 0, PTE_EXECUTABLE)?;
            }
            done = done
                .checked_add(count as u64 * GPU_PAGE_SIZE as u64)
                .ok_or(Error::Range)?;
...
impl PrimeState {
    ...
    pub fn remove(&mut self, token: u64) -> Result<()> {
        let index = self
            ...
            .ok_or(Error::NotFound)?;
        self.exports.swap_remove(index);
        Ok(())
    }
}
...
impl FramebufferState {
    ...
    pub(crate) fn pin(
        ...
        let (bo_gpu_address, bo_size, bo_place, metadata_flags, tiling_info) = {
            let guard = object.lock();
            if config.minimum_size > guard.bo.size as u64 {
                return Err(Error::InvalidArgument);
            }
            (
                guard.bo.gpu_addr,
        ...
        self.buffers.push(FramebufferRef {
            handle,
            object,
            bo_gpu_address,
            bo_size,
            ...
            meta_pitch: config.meta_pitch,
            meta_offset: config.meta_offset,
            tiling_info,
        });
        crate::dev_info!(
...
impl FileState {
    pub fn manage_context(&mut self, bytes: &mut [u8]) -> Result<(u32, u32)> {
        let file = self;
        ...
        match request.operation {
            uapi::AMDGPU_CTX_OP_ALLOC_CTX => {
                ...
                // Linux normalizes unknown priorities to NORMAL. ASTRA has no
                // software scheduler yet, so the normalized value has no
                // persistent state or observable scheduling effect.
                file.contexts.push(ContextState { id });
                bytes.fill(0);
...
impl FileState {
    ...
    fn parse_bo_entries(
        ...
        let bytes = if total == 0 {
            ...
            Self::read_user_bytes(pointer, total)?
        };
        ...
        for index in 0..count {
            ...
            entries.push(BoListEntry {
                handle,
                object: self.buffer(handle)?.object.clone(),
            });
        }
        Ok(entries)
    }
...
impl FileState {
    pub fn manage_bo_list(&mut self, bytes: &mut [u8]) -> Result<u32> {
        let file = self;
        ...
        match request.operation {
            uapi::AMDGPU_BO_LIST_OP_CREATE => {
                ...
                let entries = file.parse_bo_entries(
                    request.bo_number,
...
impl SyncobjPoint {
    fn append_chunk(chunk: uapi::CsChunk, timeline: bool, output: &mut Vec<Self>) -> Result<()> {
        ...
        let bytes = FileState::read_user_bytes(chunk.data, length)?;
        output
...
impl FileState {
    pub fn submit(&mut self, gpu: &mut Gpu, bytes: &mut [u8]) -> Result<u64> {
        let file = self;
        file.reap_submissions(gpu)?;
        ...
        let mut objects = SubmissionObjects::new();
        ...
        if request.bo_list_handle != 0 {
            let list = file
                ...
                .ok_or(Error::NotFound)?;
            objects.reserve(list.buffers.len())?;
            for entry in &list.buffers {
                objects.insert(entry.object.clone())?;
            }
        }

        let pointer_bytes = Self::read_user_bytes(
            request.chunks,
        ...
        for index in 0..request.num_chunks as usize {
            ...
            let descriptor = Self::read_user_bytes(pointer, 16)?;
            ...
            match chunk.id {
                uapi::AMDGPU_CHUNK_ID_BO_HANDLES => {
                    ...
                    let data = Self::read_user_bytes(chunk.data, uapi::BO_LIST_SIZE)?;
                    let list = uapi::BoListRequest::parse(&data)?;
                    let entries =
                        file.parse_bo_entries(list.bo_number, list.bo_info_size, list.bo_info_ptr)?;
                ...
                uapi::AMDGPU_CHUNK_ID_IB => {
                    ...
                    let data = Self::read_user_bytes(chunk.data, 32)?;
                ...
                uapi::AMDGPU_CHUNK_ID_FENCE => {
                    ...
                    let data = Self::read_user_bytes(chunk.data, 8)?;
                ...
                uapi::AMDGPU_CHUNK_ID_SYNCOBJ_IN => {
                    SyncobjPoint::append_chunk(chunk, false, &mut waits)?;
                }
                uapi::AMDGPU_CHUNK_ID_SYNCOBJ_OUT => {
                    SyncobjPoint::append_chunk(chunk, false, &mut signals)?;
                }
                uapi::AMDGPU_CHUNK_ID_SYNCOBJ_TIMELINE_WAIT => {
                    SyncobjPoint::append_chunk(chunk, true, &mut waits)?;
                }
                uapi::AMDGPU_CHUNK_ID_SYNCOBJ_TIMELINE_SIGNAL => {
                    SyncobjPoint::append_chunk(chunk, true, &mut signals)?;
                }
                uapi::AMDGPU_CHUNK_ID_DEPENDENCIES
                | uapi::AMDGPU_CHUNK_ID_SCHEDULED_DEPENDENCIES => {
                    let length = chunk.length_dw as usize * 4;
                    if length == 0 || !length.is_multiple_of(24) {
                        return Err(Error::InvalidArgument);
                    }
                    let data = Self::read_user_bytes(chunk.data, length)?;
                    dependencies
        ...
        for ib in &ibs {
            ...
            let mapping = file
                ...
                .ok_or(Error::InvalidArgument)?;
            let object = mapping.object.clone();
            if bo_list_supplied && !objects.contains(&object) {
                return Err(Error::InvalidArgument);
            }
            objects.insert(object)?;
        }

        // Linux amdgpu_cs does not cache-maintain every BO in a submission.
        // ASTRA's x86_64 GART backing is DMA coherent; userspace writes are
        // ordered before the later ring/doorbell publication.  Per-object
        // sync here added an SFENCE for every BO on every CS and serialized
        // otherwise independent compositor and application submissions.
        for dependency in dependencies {
            let _ = gpu.wait_fence(dependency, u64::MAX)?;
        }
        for wait in waits {
            file.file.wait_syncobj(wait.handle, wait.point, i64::MAX)?;
        }

        let root_pde = file.vm.root_pde(gpu)?;
        ...
        file.submissions
            .try_reserve(1)
            .map_err(|_| Error::OutOfMemory)?;
        let completion = gpu.submit_user_ibs(crate::ip::UserSubmission {
            ip_type,
            ring,
            vmid: file.vm.vmid,
            root_pde,
...
impl FileState {
    pub fn wait_submission(&mut self, adapter: &mut Adapter, bytes: &mut [u8]) -> Result<u64> {
        let file = self;
        ...
        if let Some(fence) = submission {
            status = (!adapter.wait_fence(fence, request.timeout)?) as u64;
            if status == 0 {
                file.reap_submissions(adapter)?;
            }
        }
        bytes.fill(0);
...
impl Adapter {
    fn allocate_bo(&mut self, request: uapi::GemCreateRequest) -> Result<(Bo, u64)> {
        ...
        let bo = if prefer_vram {
            match self.mem.alloc_vram_aligned(&mut self.regs, size, alignment) {
                Ok(bo) => bo,
                Err(Error::OutOfMemory) => {
                    self.mem
                        .alloc_gart_aligned(&mut self.regs, size, alignment)?
                }
...
impl FileState {
    pub fn create_bo(&mut self, adapter: &mut Adapter, bytes: &mut [u8]) -> Result<u32> {
        let file = self;
        ...
        let (bo, actual_domain) = adapter.allocate_bo(request)?;
        debug_assert_eq!(bo.size as u64, alloc_size);
        let flags = request.domain_flags | uapi::AMDGPU_GEM_CREATE_VRAM_CLEARED;
        let bo = if flags & uapi::AMDGPU_GEM_CREATE_VRAM_WIPE_ON_RELEASE != 0 {
            bo.wipe_on_release()
        } else {
            bo
        };
...
impl FileState {
    pub fn mmap_offset(&self, bytes: &mut [u8]) -> Result<u64> {
        let file = self;
...
impl FileState {
    pub fn metadata(&mut self, bytes: &mut [u8]) -> Result<()> {
        let file = self;
...
impl FileState {
    pub fn wait_bo(&mut self, adapter: &mut Adapter, bytes: &mut [u8]) -> Result<()> {
        let file = self;
        ...
        for submission in &file.submissions {
            if submission
                ...
                .iter()
                .any(|current| Arc::ptr_eq(current, &object))
                && !adapter.wait_fence(submission.fence, timeout)?
            {
                busy = true;
                break;
            }
...
impl FileState {
    pub fn operate_bo(&mut self, bytes: &mut [u8]) -> Result<()> {
        let file = self;
...
impl FileState {
    fn clear_va_range(&mut self, gpu: &mut Gpu, address: u64, size: u64) -> Result<()> {
        let file = self;
        let end = address.checked_add(size).ok_or(Error::InvalidArgument)?;
        file.vm.clear_range(gpu, address, size)?;
        let mut mappings = Vec::new();
        mappings
            .try_reserve(file.mappings.len().saturating_add(1))
            .map_err(|_| Error::OutOfMemory)?;
        for mapping in file.mappings.iter().cloned() {
...
impl FileState {
    pub fn update_va(&mut self, gpu: &mut Gpu, bytes: &[u8]) -> Result<()> {
        let file = self;
            ...
            uapi::AMDGPU_VA_OP_MAP | uapi::AMDGPU_VA_OP_REPLACE => {
                ...
                }
                {
                    ...
                    if request.operation == uapi::AMDGPU_VA_OP_REPLACE {
                        file.clear_va_range(gpu, request.va_address, request.map_size)?;
                    ...
                    let pte_flags = UserVm::pte_flags(request.flags, object.flags, object.bo.place);
                ...
                file.mappings.push(VaMapping {
                    object: file.buffer(request.handle)?.object.clone(),
                ...
                let index = file
                    ...
                    .position(|mapping| {
                        Arc::ptr_eq(&mapping.object, &object)
                            && mapping.address == request.va_address
                    })
                    .ok_or(Error::NotFound)?;
...
impl FileState {
    /// Drops only this drm_file's GEM handle, as Linux drm_gem_handle_delete()
    /// does. VM mappings, KMS framebuffers, PRIME exports and scheduler jobs
    /// each hold their own Arc, so their object lifetime is independent of the
    /// userspace handle and no GPU-wide synchronization belongs on this path.
    pub fn close_bo(&mut self, bytes: &[u8]) -> Result<u32> {
...
impl PrimeState {
    pub fn export(
        ...
        let file = files
            ...
            .ok_or(Error::NotFound)?;
        let object = file.buffer(handle)?.object.clone();
        let (physical_address, length) = {
            let object = object.lock();
            let physical_address = match object.bo.place {
                Place::Vram => adapter
                    .vram_base
                    .checked_add(object.bo.gpu_addr)
                    .ok_or(Error::Range)?,
            ...
            (physical_address, object.bo.size)
        };
...
impl PrimeState {
    pub fn import(&self, files: &mut [FileState], file_id: FileId, token: u64) -> Result<u32> {
        let prime = self;
        let object = prime
            .exports
            .iter()
...
impl FramebufferState {
    pub(crate) fn gem_handle(
        ...
        let bo_size = framebuffer.bo_size;
        let file = files
            .iter_mut()
            .find(|file| file.belongs_to(file_id))
            .ok_or(Error::NotFound)?;
        let existing_mmap_offset = file
            .buffers
            .iter()
            .find(|buffer| Arc::ptr_eq(&buffer.object, &object))
            .map(|buffer| buffer.mmap_offset);
        file.buffers
            .try_reserve(1)
            .map_err(|_| Error::OutOfMemory)?;
        let handle = file.allocate_handle()?;
        let mmap_offset = match existing_mmap_offset {
            Some(offset) => offset,
            None => file.allocate_mmap_offset(bo_size)?,
        };
        file.buffers.push(UserBuffer {
            handle,
            object,
...
impl FramebufferState {
    pub(crate) fn scanout(
        ...
        let framebuffer = framebuffers
            ...
            .ok_or(Error::NotFound)?;
        let object = framebuffer.object.clone();
        if framebuffer.bo_place != Place::Vram {
            return Err(Error::NoSpace);
        }
        let bo_gpu_address = adapter
            .gmc
            .fb_start
            .checked_add(framebuffer.bo_gpu_address)
            .ok_or(Error::Range)?;
        ...
        let meta_address = dcc_offset
            .map(|offset| {
                if offset >= framebuffer.bo_size as u64 {
                    return Err(Error::InvalidArgument);
                }
...
impl FramebufferState {
    pub(crate) fn cursor_buffer(
        ...
        let required = (framebuffer.pitch as u64)
            ...
            .ok_or(Error::InvalidArgument)?;
        if required > framebuffer.bo_size as u64 {
            return Err(Error::InvalidArgument);
        }
        if framebuffer.bo_place != Place::Vram {
            return Err(Error::NoSpace);
        }
        let gpu_address = fb_start
            .checked_add(framebuffer.bo_gpu_address)
...
impl Adapter {
    ...
    fn sdma_count(&self) -> u32 {
        [HwIp::Sdma0, HwIp::Sdma1, HwIp::Sdma2, HwIp::Sdma3]
            .into_iter()
            .filter(|ip| self.versions[ip.index()][0] != 0)
            .count() as u32
    }

    fn ip_count(&self, ip: u32) -> Result<u32> {
        let gc = self.versions[HwIp::Gc.index()][0] != 0;
        match ip {
            ...
            uapi::AMDGPU_HW_IP_DMA if self.sdma_count() != 0 => Ok(0),
...
impl Adapter {
    fn hw_ip_info(&self, ip: u32, instance: u32) -> Result<Vec<u8>> {
        if instance != 0 {
            return Err(Error::InvalidArgument);
        }
        let (version, rings, start_align, size_align) = match ip {
            uapi::AMDGPU_HW_IP_GFX => (self.versions[HwIp::Gc.index()][0], 2, 32, 32),
            uapi::AMDGPU_HW_IP_COMPUTE => (self.versions[HwIp::Gc.index()][0], 8, 32, 32),
            uapi::AMDGPU_HW_IP_DMA => (self.versions[HwIp::Sdma0.index()][0], 0, 256, 4),
            uapi::AMDGPU_HW_IP_UVD | uapi::AMDGPU_HW_IP_VCN_DEC => {
                (self.versions[HwIp::Uvd.index()][0], 0, 256, 64)
            }
            uapi::AMDGPU_HW_IP_UVD_ENC | uapi::AMDGPU_HW_IP_VCN_ENC => {
                (self.versions[HwIp::Uvd.index()][0], 0, 256, 4)
            }
            uapi::AMDGPU_HW_IP_VCN_JPEG => (self.versions[HwIp::Uvd.index()][0], 0, 256, 64),
            _ => return Err(Error::InvalidArgument),
        };
        if version == 0 {
            return Err(Error::InvalidArgument);
        ...
        Ok(HwIpInfo {
            ...
            available_rings: Self::ring_mask(rings),
...
impl Adapter {
    fn device_info(&self) -> Vec<u8> {
        let gfx = self.gfx_info;
        DeviceInfo {
            device_id: self.info.device_id as u32,
            chip_rev: 0,
            // Linux nv_common_early_init: GC 10.3.4 external_rev = rev_id + 0x3c.
            external_rev: 0x3c,
            pci_rev: self.info.revision_id as u32,
            ...
            max_engine_clock: self.clocks.max_engine_clock_khz,
            max_memory_clock: self.clocks.max_memory_clock_khz,
            ...
            gart_page_size: 4096,
            vram_type: self.gmc.vram_type,
            vram_bit_width: self.gmc.vram_width,
            ...
            min_engine_clock: self.clocks.min_engine_clock_khz,
            min_memory_clock: self.clocks.min_memory_clock_khz,
...
impl Adapter {
    fn heaps(&self) -> MemoryInfo {
        let vram_total = self.gmc.real_vram_size;
        let visible_total = self.gmc.visible_vram_size.min(vram_total);
        let gtt_total = self.gmc.gart_size;
        let vram_usage = self.mem.vram_usage().min(vram_total);
        let visible_usage = vram_usage.min(visible_total);
        let gtt_usage = self.mem.gart_usage().min(gtt_total);
        let vram_usable = vram_total.saturating_sub(self.gmc.vram_reserved_size);
...
impl Adapter {
    fn firmware_info(&self, fw_type: u32, instance: u32, index: u32) -> Result<Vec<u8>> {
        if instance != 0 {
            return Err(Error::InvalidArgument);
        }
        ...
        let firmware = self.firmware(id).ok_or(Error::InvalidArgument)?;
        let mut bytes = [0u8; 8];
...
impl Adapter {
    fn read_mmr_registers(&self, request: &InfoRequest) -> Result<Vec<u8>> {
...
impl Adapter {
    pub fn query_info(&self, request: &InfoRequest) -> Result<Vec<u8>> {
        match request.query {
            uapi::AMDGPU_INFO_ACCEL_WORKING => Ok(Self::scalar_u32(1)),
            uapi::AMDGPU_INFO_HW_IP_COUNT => Ok(Self::scalar_u32(self.ip_count(request.data[0])?)),
            uapi::AMDGPU_INFO_HW_IP_INFO => self.hw_ip_info(request.data[0], request.data[1]),
            uapi::AMDGPU_INFO_DEV_INFO => Ok(self.device_info()),
            ...
            uapi::AMDGPU_INFO_FW_VERSION => {
                self.firmware_info(request.data[0], request.data[1], request.data[2])
            }
            uapi::AMDGPU_INFO_VRAM_USAGE => Ok(Self::scalar_u64(self.mem.vram_usage())),
            uapi::AMDGPU_INFO_VIS_VRAM_USAGE => Ok(Self::scalar_u64(
                self.mem.vram_usage().min(self.gmc.visible_vram_size),
            )),
            uapi::AMDGPU_INFO_GTT_USAGE => Ok(Self::scalar_u64(self.mem.gart_usage())),
            ...
            uapi::AMDGPU_INFO_READ_MMR_REG => self.read_mmr_registers(request),
            uapi::AMDGPU_INFO_MEMORY => Ok(self.heaps().encode()),
            ...
            | uapi::AMDGPU_INFO_NUM_VRAM_CPU_PAGE_FAULTS => Ok(Self::scalar_u64(0)),
            uapi::AMDGPU_INFO_VRAM_LOST_COUNTER | uapi::AMDGPU_INFO_RAS_ENABLED_FEATURES => {
                Ok(Self::scalar_u32(0))
            }
            uapi::AMDGPU_INFO_MAX_IBS => Ok(Self::max_ibs()),
            _ => Err(Error::InvalidArgument),
        }
    }
}
