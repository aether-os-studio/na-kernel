...
use crate::dev_info;
use crate::ip::{HWIP_COUNT, HwIp, IpVersion, MAX_BASE_ADDR, MAX_INSTANCE, hw_ip_for_id};
use crate::regs::Regs;
...
pub fn parse(regs: &mut Regs) -> Result<Discovery> {
        ...
        .map(|(id, e)| (id, e.0, e.1, e.2))
    {
        ...
        let calculated = checksum16(table);
        dev_info!(
        ...
        match id {
            TABLE_IP_DISCOVERY => parse_ip_discovery(table, start, regs, &mut discovery)?,
            TABLE_GC => parse_gc_info(table, &mut discovery.gfx_info),
            TABLE_HARVEST => parse_harvest(table),
    ...
    for i in 0..num_dies as usize {
        ...
        for _ in 0..num_ips {
            ...
            dev_info!(
                ...
                hw_id,
                hw_ip_for_id(hw_id).map(|ip| ip.name()).unwrap_or("?"),
                instance,
                major,
                minor,
                revision,
            ...
            let Some(ip) = hw_ip_for_id(hw_id) else {
                continue;
            };
...
fn parse_gc_info(table: &[u8], info: &mut GfxInfo) {
