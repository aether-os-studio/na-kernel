...
pub fn disable_cursor(regs: &mut DcnCursorRegs) -> Result<()> {
...
pub fn program_cursor_attributes(
    regs: &mut DcnCursorRegs,
    ...
    let mut control = regs.read_dcn(dcn::mmCURSOR0_0_CURSOR_CONTROL, hubp)?;
    control = set_field(
        control,
    ...
    regs.write_dcn(dcn::mmCURSOR0_0_CURSOR_CONTROL, hubp, control)?;
    regs.write_dcn(
    ...
    let mut dpp_control = regs.read_dcn(dcn::mmCNVC_CUR0_CURSOR0_CONTROL, dpp)?;
    ...
    regs.write_dcn(dcn::mmCNVC_CUR0_CURSOR0_CONTROL, dpp, dpp_control)
}
...
pub fn program_cursor_position(
    regs: &mut DcnCursorRegs,
    ...
    if position.width == 0 || position.height == 0 || position.width > 256 || position.height > 256
    {
        return Err(Error::InvalidArgument);
    }
    ...
    let dst_x_offset =
        ((src_x_offset.max(0) as u64) * 50_000 / u64::from(DEFAULT_PCLK_KHZ)).min(0x1fff) as u32;
...
pub fn program_primary_geometry(regs: &mut Regs, width: u32, height: u32) -> Result<()> {
...
pub fn program_primary_surface(regs: &mut Regs, config: &PrimarySurfaceConfig) -> Result<()> {
    if config.address == 0
        || config.width == 0
        || config.height == 0
        || config.pitch == 0
    ...
    program_primary_requestor(regs, config.width, pitch_pixels, config.swizzle)?;
    ...
    // viewport banks.
    program_primary_viewports(regs, config.width, config.height)?;
    ...
    program_primary_address(regs, config.address, config.meta_address)
}
...
pub fn program_primary_address(
    regs: &mut Regs,
    address: u64,
    meta_address: Option<u64>,
) -> Result<()> {
    if address == 0 {
        return Err(Error::InvalidArgument);
    }
...
impl DcnBlock {
    ...
    fn program_hubp(&self, regs: &mut Regs, fb_start: u64) -> Result<()> {
        ...
        program_primary_requestor(regs, t.h_addressable as u32, t.h_addressable as u32, 0)?;
        ...
        program_primary_viewports(regs, t.h_addressable as u32, t.v_addressable as u32)?;
